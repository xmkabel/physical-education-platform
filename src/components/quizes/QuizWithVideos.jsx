...
import { faArrowLeft, faCheck, faBookOpen, faArrowRight, faStar, faTrophy, faListAlt, faVideo } from '@fortawesome/free-solid-svg-icons';
            ...
            {!isSubmitted ? (
              <>
                <div className="animate__animated animate__fadeIn">
                  {quizContent && quizContent.length > 0 && currentStep < quizContent.length && quizContent[currentStep] && quizContent[currentStep].type === 'video' ? (
                    <div className="reading-container mb-4">
                      <div className="d-flex align-items-center mb-3 bg-light p-2 rounded-top" style={{borderBottom: 'solid 1px var(--gold)'}}>
                        <FontAwesomeIcon icon={faVideo} className="me-2 ms-2" size="lg" style={{ color: 'var(--navy-blue)'}} />
                        <h4 className="mb-0 fs-5 fw-bold">فيديو {currentStep + 1}: {quizContent[currentStep].title}</h4>
                      </div>
                      <Card className="bg-light border-0 shadow-sm rounded-bottom">
                        <Card.Body className="p-4" style={{ textAlign: 'right', maxHeight: '400px', overflowY: 'auto' }}>
                          {<Video key={quizContent[currentStep].path} path={quizContent[currentStep].path} title={quizContent[currentStep].title} />}
                        </Card.Body>
                      </Card>
                    </div>
                ...
                <div className="question-navigation mb-4 d-flex flex-wrap gap-2 justify-content-center">
                  {quizContent.map((item, index) => {
                    ...
                    let displayNumber;
                    if (item.type === 'video') {
                      displayNumber = <FontAwesomeIcon icon={faVideo} className="me-2 ms-2" size="lg" style={{ color: currentStep === index || answers[item.id] !== undefined ? 'white' : 'var(--navy-blue)'}} />
                    } else {
